# Registers the Elixir language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koElixirLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Elixir")
    registry.registerLanguage(KoElixirLanguage())

class KoElixirLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Elixir"
    lexresLangName = "Elixir"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "b3ebfc0c-5297-4982-b23a-afdcbdbaed14"
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Elixir'}
